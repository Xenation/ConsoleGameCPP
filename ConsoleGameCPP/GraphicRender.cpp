...
GraphicRender::GraphicRender(Entity* entity, Graphic* toRender) : entity(entity), toRender(toRender) {
...
void GraphicRender::Render(CHAR_INFO buffer[SCREEN_HEIGHT][SCREEN_WIDTH]) {
	...
	for (int y = 0; y < toRender->height; y++) {
		for (int x = 0; x < toRender->width; x++) {
			...
			buffer[screenPos.y + y][screenPos.x + x].Char.AsciiChar = toRender->contents[y][x];
